...
public class tab2 extends Fragment {
    ...
    private void startActivity() {
        ...
        Cursor c = db.query("mytable", null, "learning = 1", null, null, null, "word ASC");
        if (c.moveToFirst()) {
            list.clear();
            do {
                Word word = new Word();
        ...
        c.close();
        //ArrayAdapter<String> adapter = new ArrayAdapter<>(this, android.R.layout.simple_list_item_multiple_choice, wordList);
        ArrayAdapter<String> adapter = new ArrayAdapter<String>(getActivity(), android.R.layout.simple_list_item_multiple_choice, wordList);
                    ...
                    case R.id.btnDeleteFromList:
                        for (int i = 0; i < sbArray.size(); i++) {
                            ...
                            if (sbArray.get(key)){
                                ...
                                Toast.makeText(getActivity(), "tab2Delete", Toast.LENGTH_LONG).show();
                            }
                        }
                        wordList.clear();
                        startActivity();
